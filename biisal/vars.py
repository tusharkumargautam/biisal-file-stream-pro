...
class Var(object):
    ...
    name = str(getenv('name', 'gangplayer'))
    SLEEP_THRESHOLD = int(getenv('SLEEP_THRESHOLD', '60'))
    WORKERS = int(getenv('WORKERS', '4'))
    BIN_CHANNEL = int(getenv('BIN_CHANNEL', '-1002211673711'))
    NEW_USER_LOG = int(getenv('NEW_USER_LOG', '-1002191829431'))
    PORT = int(getenv('PORT', '8080'))
    BIND_ADRESS = str(getenv('WEB_SERVER_BIND_ADDRESS', '0.0.0.0'))
    PING_INTERVAL = int(environ.get("PING_INTERVAL", "1200"))  # 20 minutes
    OWNER_ID = [int(x) for x in os.environ.get("OWNER_ID", "7442164628").split()]
    ...
    if 'DYNO' in environ:
        ...
        ON_HEROKU = False
    FQDN = str(getenv('FQDN', 'teradisk.me')) if not ON_HEROKU or getenv('FQDN', '') else APP_NAME+'.herokuapp.com'
